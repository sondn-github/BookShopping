			...
			<div class="main-content">
				...
				<div class="row">
					<div class="col-sm-12">
							...
							<div class="row">
								...
								<div class="col-sm-3">
									<div class="single-item">
										...
										<div class="single-item-caption">
											<form method="POST">
												<button class="add-to-cart pull-left" type="submit" name=<?php echo "hot".$row['book_id']?>><i class="fa fa-shopping-cart"></i></button>
											</form>

									<!-- Khi click vào biểu tg giỏ hàng thì tăng số lượng sách lên 1 đơn vị và tải lại trang home.php-->
									<?php 

										if (isset($_POST['hot'.$row['book_id']]) && !isset($_SESSION['user_fullname'])) { ?>
									    	<script language="javascript">                           
   												alert('<?php echo "Bạn chưa đăng nhập. Ấn vào \"OK\" để đăng nhập!!!"  ?>');                                                 
											</script>
									<?php
										$url = "login.php";
										echo "<meta http-equiv='refresh' content='0;url=$url' />";
										}elseif (isset($_POST['hot'.$row['book_id']]) && isset($_SESSION['user_fullname'])) {
											if (!isset($_SESSION['bID'.$row['book_id']]) && ($_SESSION['sl'.$row['book_id']] >= 1)) {
												$_SESSION['bID'.$row['book_id']] = 1;
											} elseif($_SESSION['sl'.$row['book_id']] > $_SESSION['bID'.$row['book_id']]) {
												$_SESSION['bID'.$row['book_id']]++;
											}elseif (isset($_POST['hot'.$row['book_id']]) && (($_SESSION['sl'.$row['book_id']] <= $_SESSION['bID'.$row['book_id']]) || ($_SESSION['sl'.$row['book_id']] == 0))) {
									?>
									    	<script language="javascript">                           
   												alert('<?php echo "Số lượng không đủ."  ?>');                                                 
											</script>
						...
						</div> <!-- .beta-products-list Sách bán chạy-->
							...
							<div class="row">
								...
								<div class="col-sm-3">
									<div class="single-item">
										...
										<div class="single-item-caption">
											<form method="POST">
												<button class="add-to-cart pull-left" type="submit" name=<?php echo "all".$row['book_id']?>><i class="fa fa-shopping-cart"></i></button>
											</form>

									<!-- Khi click vào biểu tg giỏ hàng thì tăng số lượng sách lên 1 đơn vị và tải lại trang home.php-->
									<?php 

										if (isset($_POST['all'.$row['book_id']]) && !isset($_SESSION['user_fullname'])) { ?>
									    	<script language="javascript">                           
   												alert('<?php echo "Bạn chưa đăng nhập. Ấn vào \"OK\" để đăng nhập!!!"  ?>');                                                 
											</script>
									<?php
										$url = "login.php";
										echo "<meta http-equiv='refresh' content='0;url=$url' />";
										}elseif (isset($_POST['all'.$row['book_id']]) && isset($_SESSION['user_fullname'])) {
											if (!isset($_SESSION['bID'.$row['book_id']]) && ($_SESSION['sl'.$row['book_id']] >= 1)) {
												$_SESSION['bID'.$row['book_id']] = 1;
											} elseif($_SESSION['sl'.$row['book_id']] > $_SESSION['bID'.$row['book_id']]) {
												$_SESSION['bID'.$row['book_id']]++;
											}elseif (isset($_POST['all'.$row['book_id']]) && (($_SESSION['sl'.$row['book_id']] <= $_SESSION['bID'.$row['book_id']]) || ($_SESSION['sl'.$row['book_id']] == 0))) {
									?>
									    	<script language="javascript">                           
   												alert('<?php echo "Số lượng không đủ."  ?>');                                                 
											</script>
